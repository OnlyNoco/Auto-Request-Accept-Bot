...
ALLOW_ALL_USERS = True # True will let all users to make invitelink with auto request support and false means only owner can generate invite link 
SETTINGS_MSG = os.environ.get(
    "SETTINGS_MSG",
    "<blockquote>Hello {mention}</blockquote>\n\n"
    "<blockquote>ᴀʟʟᴏᴡ ᴀʟʟ ᴜsᴇʀs: {allow_all_users}</blockquote>\n"
    "<blockquote expandable>This will allow all users to use this bot on their channel and users can generate channel invite links also with auto approve support</blockquote>"
)
